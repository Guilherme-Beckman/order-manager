	...
	public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http) {
		return http.csrf(csrf -> csrf.disable())
				.authorizeExchange(exchange -> exchange
						...
						.permitAll()
						.pathMatchers("/stores/addProduct", "/stores/menu/**", "orders/store", "orders/store/status/**")
						.hasRole("STORE")
						.pathMatchers("/auth/inviteResetPassword", "/auth/newPassword", "products/review/**",
								"/orders/cart/add/**", "/orders/cart/remove/**", "/orders/make/**", "orders/cancel/**",
								"/orders/client/**")
						.hasRole("USER").pathMatchers("/auth/sendcode", "/auth/validate").hasRole("NON_VERIFIED_EMAIL")
						.pathMatchers("/clients/addAddress", "/clients/perfil").hasRole("VERIFIED_EMAIL").anyExchange()
						.authenticated())
