...
public class DecryptAddresses {
	...
	public List<AddressDTO> decryptAddresses(List<String> addressesId) {
		...
		addressesId.forEach(id -> {
			...
			if (!street.equals("null")) street = this.cryptoUtils.decrypt(street);
			if (!number.equals("null")) number = this.cryptoUtils.decrypt(number);
			if (!complement.equals("null")) complement = this.cryptoUtils.decrypt(complement);
			if (!neighborhood.equals("null")) neighborhood = this.cryptoUtils.decrypt(neighborhood);
			if (!city.equals("null")) city = this.cryptoUtils.decrypt(city);
			if (!state.equals("null")) state = this.cryptoUtils.decrypt(state);
			if (!zipCode.equals("null")) zipCode = this.cryptoUtils.decrypt(zipCode);
