...
public class GetProductByIdProducer {
	...
	public ProductModelDTO requestProductsByIdProducer(String id) {
		...
		LinkedHashMap<String, Object> linkedHashMapProduct = (LinkedHashMap<String, Object>) rabbitTemplate.convertSendAndReceive(RabbitMQConfig.PRODUCT_BY_ID_EXCHANGE,
				RabbitMQConfig.PRODUCTS_BY_ID_KEY, message);
		System.out.println(linkedHashMapProduct);
	    ...
	    private ProductModelDTO mapToProductModelDTO(LinkedHashMap<String, Object> map) {
	        ...
	        return new ProductModelDTO(
	            ...
	            (String) map.get("menuId"),
	            (String) map.get("name"),
	            (Integer) map.get("price"),
	            (String) map.get("description"),
	            (Boolean) map.get("availability"),
	            (Float) rating,
	            (Integer) map.get("reviewsCount"),
